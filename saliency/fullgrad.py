...
class FullGrad():
    ...
    def checkCompleteness(self):
        """
        Check if completeness property is satisfied. If not, it usually means
        that some bias gradients are not computed
        (e.g.: implicit biases of non-linearities).
        """
        ...
        input_grad, bias_grad = self.fullGradientDecompose(input,
                                                           target_class=None)
        ...
        assert isclose(raw_output.max().item(),
                       fullgradient_sum.item(),
    ...
    def fullGradientDecompose(self, image, target_class=None):
        ...
        if target_class is None and len(out.size()) == 1:
            target_class = out.data.max(0, keepdim=True)[1]
        else:
        ...
        output_scalar = -1. * F.nll_loss(out, target_class.flatten(),
                                         reduction='sum')

        input_gradient, feature_gradients = \
            self.model_ext.getFeatureGrads(image, output_scalar)
    ...
    def saliency(self, image, target_class=None):
        ...
        input_grad, bias_grad = \
            self.fullGradientDecompose(image, target_class=target_class)

        # Input-gradient * image
        # grd = input_grad * image
        # gradient = self._postProcess(grd)
        # cam = gradient

        # No Conv layers in our case, skip bias_grad
        # im_size = image.size()
        # print(im_size)
        ...
        #         gradient = F.interpolate(temp, size=(im_size[2], im_size[3]),
